...
class BlogPostDAO:
	...
	try:
	    ...
	    #last_error = self.posts.update({"permalink":permalink,"comment_ordinal":comment_ordinal},{"$inc": {"comments.$.num_likes":1}})
	    last_error = self.posts.update({"permalink":permalink},{"$inc": {"comments." + str(comment_ordinal)+ ".num_likes":1}})
	    return last_error['n']

	except:

            print "Could not alter num_likes, error"
            print "Unexpected error:", sys.exc_info()[0]
            return 0
